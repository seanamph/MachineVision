...
{
	u_char	**list0, **MaskList;

	list0 = (u_char**)Create2DList(pImg);
	MaskList = (u_char**)Create2DList(pMaskImg);
...
int NGauge::SubPixel_core(u_char **list0, u_char **MaskList, int wid, int hei, double *sub_x, double *sub_y)
{
	int  	i, j, Dx, Dy, Di;
...
u_char** NGauge::Create2DList(NImage *pImg)
{
	int			i;
	u_char		**list;

	list = (u_char**)malloc(pImg->GetWidth() * sizeof(u_char*));
...
void NGauge::Release2DList(u_char** list)
{
	free(list);
}