...
int	NObject::Contour_Tracing(NImage* pImg, int blob_num, int *ct_x, int *ct_y)
{
	...
	int x = m_chain[0];														// ��X�����e�T�Ӥ������O�N�� �_�l�y��X�B�y��Y�P��X�ƶq
	int y = m_chain[1];
...
bool NObject::CreateMaskFromObject(NImage* pMask_Img, int blob_num)
{
	...
	list = (BYTE**)Create2DList(pMask_Img);
...
BYTE** NObject::Create2DList(NImage *pImg)
{
	int			i;
	BYTE		**list;
...
void NObject::Release2DList(BYTE** list)
{
	free(list);
}

int NObject::LineCoding(BYTE **list, struct TAB *tab, int area[][2], int &tab_size, int wid, int hei)
{
	int		num, an;
...
int NObject::CreateTab(BYTE **list, int x, int y, int Dx, int Dy, struct TAB *tab)
{
	int   i, j, k, f, num;
...
int NObject::SingleTrack(BYTE **list, int i, int j, int iop, int *code, int flag, int wid, int hei)
{
	int  kw, code_n;
	int  curr[2], next[2];
