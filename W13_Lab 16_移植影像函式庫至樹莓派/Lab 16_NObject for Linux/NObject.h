...
private:
	...
	int		LineCoding(u_char **list, struct TAB *tab,int area[][2], int &tab_size, int wid, int hei);	// 連結體線段編碼
	...
	int		CreateTab(u_char **list, int x, int y, int wid, int hei, struct TAB *tab);  	// 建立線段表並初始化(線段編碼步驟1)
	...
	int		SingleTrack(u_char **list, int i, int j, int iop, int *code, int flag,
...
public:
	...
	bool	Rectangle(int blob_num, int *start_x, int *start_y, int *rect_w, int *rect_h);// 計算外接矩形
